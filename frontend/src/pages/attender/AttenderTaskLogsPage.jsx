...
export default function AttenderTaskLogsPage({ token }) {
  ...
  const loadLogs = async (staffId, staffName = '', useFilter = false) => {
    try {
      ...
      const filters = useFilter
        ...
        : {};
      setRows(await api.staffTaskLogHistory(token, staffId, filters));
      setMessage('');
    } catch (err) {
      setMessage(err.message);
    }
    ...
    <div className="stack">
      ...
      {selectedId && (
        <section className="card">
          ...
          {rows.length === 0 && <p className="muted">No tasklogs found for this staff.</p>}
